...
Para la selección de la variable explicativa eliminamos las variables de carácter, y dejamos las variables numéricas. Dividimos las variables en 3 grupos para poder analizar las correlaciones de las variables respecto el precio, este fue el resultado de cada set:
...
Dado el anterior gráfico, podemos observar que no existe un patrón notório en el gráfico, ya sea como la silueta de una recta, con esto podemos asumir que hay heterocedisticidad, por lo tant la varianza de nuestros errores es constante.
...
Esto resultado lo obtenemos de la siguiente forma: Obtenemos la suma de cuadrados de los errores (MSE), de la tabla ANNOVA, después calculamos la raiz cuadrada del MSE.

Ya con (MSE)^(1/2), podemos obtener la división de los residuales entre la raíz de MSE y compararlos con nuewstras x'i, estas variables las metemos en un DF y graficamos las diferencias.
...
Utilizamos las variables numéricas nuevamente para poder tener un mejor control sobre las regresoras y utilizamos el método "Backward", este método es computacional, pero debido a que hay una cantidad manejable de variables lo hicimos a mano, tomando en cuanta un nivel de significancia de .005 
...
Con los datos que obtuvimos anteriormente, podemos observar que algunos de los P-values de las regresoras son mayores a 0.005, por lo que iremos depurando las variables que no cumplen con la prueba Global de la regresión, e iremos corriendo la regresión nuevamente por cada modelo.
...
Una vez que eliminamos estos datos, volvemos a realizar el este gráfico para confirmar que no existan datos atípicos.
